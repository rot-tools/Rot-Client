...
| Automated baseline | Current working tree: **2,415** shared tests plus **205** Plus tests; 0 failures or errors. Shared and Plus client compilation, `verifyLegitJar`, `verifyDungeonJarBoundary`, `check`, and `clean build` pass. SHA-256: Rot Client `FA794083ECA31B65B148952280ECBD63DE18DA693EBB3FA2D419DA0E4AAE767A`; Rot Client+ `547DB76597933D5230066A33356CA4EBBAB521BCDBFAC89F45D5705901E7CB2B`. Minecraft runtime validation remains pending. |
| Current phase | Lite/Plus packaging separation is in progress; **Lite is not Modrinth-ready**. Dual catalog: Lite **83** parents / Plus **135**. The entire Dungeons catalog group, full `DungeonRuntime`, full `DungeonPuzzlePolicy`, puzzle-board data, and Dungeons example profile are Plus-only. Lite carries inert compatibility classes and no dungeon cards. Mixed F7 and related policy classes still need code/bytecode review; the wider Lite catalog needs a multiplayer-rule review, separate Minecraft smoke tests, and code/asset/AI provenance sign-off. See `docs/MODRINTH_LITE_RELEASE.md`. |

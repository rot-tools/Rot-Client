...
| Current QoL / session checkpoint | Dual editions from one repo: **Rot Client** (91 HUD/QoL parents) and **Rot Client+** (135 parents). Trajectories, World Scanner, Mob Highlight, Dungeon ESP, Terminal Solver, Dungeon Puzzles, Auto Sprint, Experiment Solver, Diana, Terminal Click Trails, Terminal Simulator, and Fishing Hotspot radar/tracer are Plus-only in the catalog and are inert in Lite even when an older profile has values saved. F7 wrong-click cancellation, Dungeon Breaker block rewriting/secret-skip, terminal wrong-slot blocking, clicked-slot hiding, and solver tooltip suppression compile only into Plus. Etherwarp depth-off is Plus-only; Lite always respects occlusion. Commission tab parser accepts `COMPLETED`, bare `N%`, and `Commission Progress` headers. Minecraft runtime validation remains pending. |
| Minecraft | 26.2 |
| Mod | 2.0.1+mc26.2 |
...
| Automated baseline | Current working tree: **2,434** shared tests plus **183** Plus tests; 0 failures or errors. Shared and Plus client compilation, `verifyLegitJar`, `check`, and `clean build` pass. SHA-256: Rot Client `FEEE18323320CCB08E08050224F28F125D53F0242E4286309026B0A8A56EF0FE`; Rot Client+ `5EA05F19849D25C1EB54B1978DB832A34983958BE52F505C2AD4687A67DCE466`. Minecraft runtime validation remains pending. |
| Current phase | Lite/Plus packaging separation is in progress for the Modrinth/CurseForge release candidate. Dual catalog: Lite **91** parents across 13 populated groups / Plus **135** across 14 groups. Dungeon ESP, Terminal Solver, and Dungeon Puzzles catalog definitions now live in Plus source. F7 Simon, Arrow Align, I4, Sharp Shooter, and wither/dragon/gate/relic world-overlay settings are Plus-only in the catalog and their reviewed runtime entry points are gated in Lite. The Diorite block-rewrite route is also Plus-gated. Other Plus features remain separated as listed above. The dungeon implementation still shares runtime classes with the legitimate HUD and requires a physical bytecode split and full feature audit before release claims are final. Minecraft smoke testing and release metadata remain. |

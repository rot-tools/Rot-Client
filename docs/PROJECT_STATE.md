...
| Automated baseline | Current working tree: **2,434** shared tests plus **183** Plus tests; 0 failures or errors. Shared and Plus client compilation, `verifyLegitJar`, `check`, and `clean build` pass. SHA-256: Rot Client `031DB0159F9BD17B41A00BD259226839842C08493D46BA29AA05520636A9FF7A`; Rot Client+ `B44779A093F07D5B6BB1CD9A001D94D785FBF7E7D68D141ABC1C5C724545B169`. Minecraft runtime validation remains pending. |
| Current phase | Lite/Plus packaging separation is in progress for the Modrinth/CurseForge release candidate. Dual catalog: Lite **91** parents across 13 populated groups / Plus **135** across 14 groups. Dungeon ESP, Terminal Solver, and Dungeon Puzzles catalog definitions now live in Plus source. F7 Simon, Arrow Align, I4, and Sharp Shooter settings are Plus-only in the catalog and their reviewed runtime entry points are gated in Lite. Other Plus features remain separated as listed above. The dungeon implementation still shares runtime classes with the legitimate HUD and requires a physical bytecode split and full feature audit before release claims are final. Minecraft smoke testing and release metadata remain. |

...
| Automated baseline | Minecraft 26.3: **2,417** shared tests plus **205** Plus tests, 0 failures or errors; shared and Plus client compilation, `verifyLegitJar`, `verifyDungeonJarBoundary`, and `clean build` pass. Both editions passed a local Minecraft startup smoke test through resource and sound initialization. Feature-level gameplay validation remains pending. |

...
| Automated baseline | Current working tree: **2,429** shared tests plus **190** Plus tests; 0 failures or errors. Shared and Plus client compilation, `verifyLegitJar`, `check`, and `clean build` pass. SHA-256: Rot Client `891071F8EC4A7CC857CD3EF58DA06ADC2EDD94CE547089573F7F4C8749945D8B`; Rot Client+ `7D71B5DC4A3F48B7148AB96DC37F6C1E0A7EFBCDCBE698EE7BD922F97A860D94`. Minecraft runtime validation remains pending. |
| Current phase | Lite/Plus packaging separation is in progress; **Lite is not Modrinth-ready**. Dual catalog: Lite **91** parents / Plus **135**. Dungeon ESP, Terminal Solver, and Dungeon Puzzles catalog parents are Plus-only. Terminal click solution code and Ice Path and Tic Tac Toe policies/scans now compile only in Plus. Lite forces saved Reveal Hidden dungeon-map mode to Explored and hides map-mode, Melody, and quiz solver controls in its HUD inspector. Other shared dungeon solver code remains in `DungeonRuntime` and mixed policies. The wider Lite catalog still needs a multiplayer-rule review, plus Minecraft smoke tests and code/asset/AI provenance sign-off. See `docs/MODRINTH_LITE_RELEASE.md`. |

...
| Automated baseline | Current working tree: **2,182** shared tests plus **180** Plus tests; 0 failures or errors. Shared and Plus client compilation, `verifyLegitJar`, and `clean build` pass. SHA-256: Rot Client `589BA04C1DAE2B1F75B79FD8F228FD66606AB8211C0F4DB8AC3AAF426DD97BD5`; Rot Client+ `E11CD7358274BEA3A11737AEBC173027FE8F2DEEFEC79457E4D907BC50EC5529`. Prism copies match. Minecraft runtime validation remains pending. |
...
| Latest edition split | Auto Clicker typed config, whitelists, CPS HUD toggle and pose are Plus-owned. Legacy root values survive in opaque `extensionFields` across config and profile saves. Lite's verifier now rejects the moved typed identifiers. Cheater Wardrobe, Superboom, and four shared dungeon parents remain in Lite audit scope. Minecraft runtime retest pending. |
| Latest runtime split | Auto Superboom click-trigger state, cooldown and timed hotbar swap-back now compile only in Plus. The Lite verifier rejects those identifiers. Superboom's typed F7 settings and the larger shared dungeon solver/UI paths remain in audit scope. |

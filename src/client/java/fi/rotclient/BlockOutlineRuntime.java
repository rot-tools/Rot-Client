 ...
 * as a rainbow whose hue flows along the edges. Called from the vanilla shape
 * outline renderer once per submitted outline.
 */
public final class BlockOutlineRuntime {
    ...
    public static boolean draw(
            ...
            float vanillaWidth) {
        // Only the main outline pass. The high-contrast backing pass keeps its own look.
        if (renderType != RenderTypes.lines()) {
            return false;
        }
        ...
        if (!BlockOutlinePolicy.isRainbow(extras.blockOutlineMode)) {
            shape.forAllEdges((x1, y1, z1, x2, y2, z2) -> {
                ...
                vertex(builder, pose, normal, width, x1, y1, z1, solid);
                vertex(builder, pose, normal, width, x2, y2, z2, solid);
            });
            return true;
        }
        int alpha = solid >>> 24;
        ...
        shape.forAllEdges((x1, y1, z1, x2, y2, z2) -> {
            ...
            for (int i = 1; i <= pieces; i++) {
                ...
                vertex(builder, pose, normal, width, px, py, pz, pieceColor);
                vertex(builder, pose, normal, width, nx, ny, nz, nextColor);
                px = nx;
                py = ny;
                pz = nz;
